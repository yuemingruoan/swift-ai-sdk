...
这份文档只描述当前 `v0.1.1` 范围内已经存在的公开 SDK surface，聚焦两个宿主侧最常
直接关心的问题：

- SDK-facing 错误分层
- OpenAI / Anthropic 直连 HTTP transport 共用的配置面

它的目标是把现状说清楚，而不是为 `v0.1.1` 之后的能力提前占位。这里没写到的东西，
不应该被当成这个版本已经承诺的能力。
...
| Runtime | `AgentRuntimeError` | 高于原始 transport 的多步运行时编排失败。 | tool loop 迭代预算耗尽。 |
...
| Conversion-specific | `OpenAIConversionError`、`AnthropicConversionError`、`OpenAIRealtimeMessageConversionError` | 刻意保留为 provider-specific 的形状转换失败。 | 不支持的 message role、不支持的内容块、无效 function call 参数。 |
...
- 未来的 middleware / policy 拦截面
- 超出当前 request ID 与 user-agent 之外的 observability API
- Anthropic streaming 配置，因为它还不在当前 SDK surface 里
