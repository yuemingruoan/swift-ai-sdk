...
| Runtime | `AgentRuntimeError` | 高层运行时编排失败，例如 tool loop 迭代次数超限。 |
...
| Conversion-specific | `OpenAIConversionError`、`AnthropicConversionError`、`OpenAIRealtimeMessageConversionError` | provider 形状转换失败，保持 provider-specific，不并入共享 SDK 错误分层。 |
...
- conversion-layer 的失败仍刻意保留为 provider-specific 的 `OpenAIConversionError`、`AnthropicConversionError` 和 `OpenAIRealtimeMessageConversionError`
