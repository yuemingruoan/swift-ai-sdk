...
- `v0.3.0` 已于 2026-04-22 发布，作为当前公开 SwiftPM 基线
- `v0.1.0` 仍然是首个公开 SwiftPM tag
- `main` 是后续开发线，用于承接 `v0.4.0` 及之后的工作
...
- 已具备 OpenAI / Anthropic 官方内建 web search tool 的 provider-native request 建模
...
| realtime | 已支持 | 暂未支持 |
| tool loop | 已支持 | 已支持 |
| 官方 web search tool | 已支持，内建 `web_search` tool | 已支持，内建 `web_search_*` server tool |
...
| 模块 | 作用 | 层级 |
| --- | --- | --- |
| `OpenAIResponsesAPI` | OpenAI Responses 原生 request/response 模型、内建 tools、builder、transport 与原始 streaming event | 底层 |
| `OpenAIAgentRuntime` | OpenAI 高层 projection、turn runner、tool-loop 编排与 runtime streaming helper | 高层 |
| `AnthropicMessagesAPI` | Anthropic Messages 原生 request/response 模型、内建 tools、builder、transport 与原始 streaming event | 底层 |
| `AnthropicAgentRuntime` | Anthropic 高层 projection、turn runner、tool-loop 编排与 streaming helper | 高层 |
| `OpenAIAuthentication` | token provider、兼容层 transform 与 authenticated OpenAI-compatible transport | 集成层 |
| `OpenAIAppleAuthentication` | Apple 平台安全 token 存储 adapter | Adapter |
| `AgentPersistence` | session/turn store、持久化 record 和 recording wrapper | 独立模块 |
| `AgentMacros` | `@Tool` 宏支持 | 编写辅助层 |

`AgentCore` 仍作为包内实现层存在，但不再作为公开 product 暴露。

## 安装

`v0.3.0` 是当前公开 SwiftPM 基线：
...
如果你正在从 `v0.2.0` 风格的 imports 迁移到 `v0.3.0` 的公开模块面，
请先看 [docs/MIGRATING_TO_V0_3_0.zh-CN.md](docs/MIGRATING_TO_V0_3_0.zh-CN.md)。
...
- OpenAI `web_search` 与 Anthropic `web_search_*` 官方内建搜索工具的 provider request 建模；适用于 provider 自带 server-side 搜索，而不是 SDK 自己执行 remote tool
- `AnthropicMessageResponse` 与原始 streaming event 会保留 Anthropic 的 `server_tool_use`、`web_search_tool_result`、text citation，以及 `usage.server_tool_use.web_search_requests`；`AnthropicAgentRuntime` 则会有意把这些 provider 内建轨迹收敛成 assistant text，除非你显式使用 raw API 面
- `AnthropicMessageResponse.webSearchOutput()`，用于在不离开底层 API 面的前提下，拿到类似 Claude Code 风格的 Anthropic web search 结构化摘要
...
- `SessionRunnerExample`：通过新的 runtime-facing import surface 演示 provider-neutral 多轮状态
- `PersistenceExample`：通过独立 `AgentPersistence` product 演示持久化 turn 记录
...
Anthropic web search 的 live smoke 依赖当前配置的后端**真实支持**
Anthropic `web_search_*` server tool。某些兼容 OpenAI 的 relay backend
即使接受请求，也可能超时，或者把响应扁平化成非官方的返回形状，而不是返回
...
- 新公开模块面的 breaking migration 说明见 [docs/MIGRATING_TO_V0_3_0.zh-CN.md](docs/MIGRATING_TO_V0_3_0.zh-CN.md)
